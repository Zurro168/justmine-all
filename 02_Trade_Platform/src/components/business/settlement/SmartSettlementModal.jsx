...
import { FileCheck, Calculator, AlertTriangle, X, CheckCircle2, Download } from 'lucide-react';
import ordersData from '../../../data/orders.json';

const SmartSettlementModal = ({ order, rules = ordersData.rules.zircon, onClose }) => {
  const basePrice = order?.basePrice || 17200;
  const wetWeight = parseInt(order?.volume || '0');

  const [ciqData, setCiqData] = useState({
    moisture: 4.5,
    mainGrade: 64.8,
    impurities: { fe: 0.12, ti: 0.15 }
  });
  const calcResult = useMemo(() => {
    ...
    const gradeDiff = ciqData.mainGrade - rules.baseGrade;
    const gradeAdjustment = gradeDiff >= 0
      ? (gradeDiff / 0.1) * rules.gradePremium
      : (gradeDiff / 0.1) * rules.gradePenalty;
    ...
    return {
      ...
      finalPrice,
      finalTotalValue: finalPrice * dryWeight,
      penalties: { fe: fePenalty, ti: tiPenalty }
    };
  }, [ciqData, wetWeight, basePrice, rules, order]);

  if (!order || !calcResult) return null;
            ...
            <div className="space-y-6 relative">
              ...
              <div className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-100">
                <div>
                  ...
                  {ciqData.moisture > rules.baseMoisture && <p className="text-xs text-orange-600 mt-1">超水阈值限制，重量折算受影响</p>}
                </div>
