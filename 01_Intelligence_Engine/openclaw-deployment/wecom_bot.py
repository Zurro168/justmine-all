                        ...
                        try:
                            ...
                            if ext in [".jpg", ".jpeg", ".png", ".bmp"]:
                                ...
                                if ocr_data and audit_service:
                                    ai_reply = build_document_audit_report(user_id, ocr_data, matched_category, f"重新识别文件 {file_title}", saved_path)
                                else:
                    ...
                    try:
                        ...
                        if info["is_single_doc"]:
                            ...
                            if ext in [".jpg", ".jpeg", ".png", ".bmp"]:
                                ...
                                if ocr_data and audit_service:
                                    ai_reply = build_document_audit_report(user_id, ocr_data, category, f"图片识别 {file_title}", saved_path)
                                else:
                                    ai_reply = f"⚠️ 视觉 OCR 提取失败或审核模块出错，请人工核实文件：{saved_path}"
                            else:
                                ...
                                try:
                                    ...
                                    # 运行多文档全量比对审核
                                    if ocr_data and audit_service:
                                        ai_reply = build_document_audit_report(user_id, ocr_data, category, f"文档解析 {file_title}", saved_path)
                                    else:
                            ...
                            def _async_manual_process_xml():
                                try:
                                    ...
                                    if ext in [".jpg", ".jpeg", ".png", ".bmp"]:
                                        ocr_data = call_vision_ocr(saved_path, matched_category)
                                        if ocr_data and audit_service:
                                            push_reply = build_document_audit_report(user_id, ocr_data, matched_category, f"重新识别文件 {file_title}", saved_path)
                                        else:
