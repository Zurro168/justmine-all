...
async def sync_index_history(index_code, name, start_years=3):
    end_date = datetime.now().strftime("%Y-%m-%d")
    start_date = (datetime.now() - timedelta(days=365*start_years)).strftime("%Y-%m-%d")
    
    all_data = []
    page = 1
    print(f"[{datetime.now()}] Async Syncing {name} ({index_code}) from {start_date} to {end_date}...")
    
    while True:
        data = await fetch_page(index_code, page, start_date, end_date)
...
async def run_scout_collection(years=3):
    os.makedirs(DATA_DIR, exist_ok=True)
    all_indices_df = []
    
    for code, name in INDICES.items():
        df = await sync_index_history(code, name, years)
        if df is not None:
    ...
    if all_indices_df:
        ...
        print(f"[{datetime.now()}] Successfully updated {HISTORY_FILE}. Total records: {len(final_df)}")
        return True
    return False

if __name__ == "__main__":
    import asyncio
    asyncio.run(run_scout_collection(years=HISTORY_YEARS))
